  ...
  <div class="designer">
    <div class="designer-table">
      <svg
        viewBox="0 0 254 127"
        class="table-svg"
        @pointerup.self="onTableDrop"
        @pointermove.self="onTableDrag"
      >
        <!-- 台面 -->
        ...
        <!-- 已放置的球 -->
        <g
          v-for="(ball, idx) in modelValue"
          :key="ball.number"
          class="ball-on-table"
          @pointerdown.stop.prevent="onBallPointerDown(idx, $event)"
          @pointermove.stop.prevent="onBallDrag(idx, $event)"
        ...
        >
          <circle
            ...
            :cy="ball.y"
            r="4.5"
            :fill="getBallColor(ball.number)"
            :stroke="selectedIdx === idx ? '#fff' : '#333'"
            :strokeWidth="selectedIdx === idx ? 2 : 0.5"
          />
          <text
            v-if="ball.number > 0"
            :x="ball.x"
    ...
    >
      <svg viewBox="0 0 20 20" width="36" height="36">
        <circle
          ...
          cy="10"
          r="8"
          :fill="getBallColor(draggingNumber)"
          stroke="#333"
          stroke-width="0.5"
        />
        <text
          v-if="draggingNumber > 8"
          x="10"
          y="10.5"
          text-anchor="middle"
          fill="#fff"
        ...
        <text
          v-if="draggingNumber <= 8 && draggingNumber > 0"
          x="10"
          y="10.5"
          text-anchor="middle"
          fill="#fff"
...
function getBallColor(n) {
  return ballColors[n] || '#999'
}

function getTableEl() {
  return (
    document.querySelector('.table-svg')?.closest('.designer-table') ||
    document.querySelector('.designer-table')
  )
}
...
function svgToPercent(clientX, clientY) {
  ...
  const x = Math.max(
    0,
    Math.min(100, ((clientX - rect.left - 4) / innerW) * 100)
  )
  const y = Math.max(
    0,
    Math.min(100, ((clientY - rect.top - 4) / innerH) * 100)
  )
  return { x, y }
}
...
function onTableClick(e) {
  if (dragging.value) {
    ...
    emit('update:modelValue', [
      ...props.modelValue,
      { number: draggingNumber.value, x: pos.x, y: pos.y },
    ])
