    ...
    <div v-else>
      ...
      <div class="hero">
        <div class="hero-top">
          <div class="hero-left">
            <span class="hero-greeting"
              >Hi，{{ store.userInfo?.nickName || '球友' }} 👋</span
            >
        ...
        <div class="hero-card">
          <div class="hero-card-row">
            ...
            <div class="hero-stat">
              <span class="hero-stat-num"
                >{{ store.todaySummary.hitRate
                }}<span class="hero-stat-percent">%</span></span
              >
      ...
      <div class="section">
        <div class="section-title">
          ...
          <span>{{
            store.todaySummary.count > 0 ? '今日训练' : '开始今天的训练'
          }}</span>
        </div>
        ...
        <div v-else class="empty-train">
          ...
          <button
            ...
            @click="$router.push('/record?from=training')"
          >
            开始训练
          </button>
        </div>
      ...
      <div class="section" v-if="weekData.length > 0">
        <div class="section-header">
          ...
          <span class="section-more" @click="$router.push('/stats')"
            >查看全部 ›</span
          >
        </div>
          ...
          <div class="bar-col" v-for="(item, i) in weekData" :key="i">
            <div class="bar-track">
              <div
                class="bar-fill"
                :class="
                  item.rate >= 70 ? 'good' : item.rate >= 50 ? 'mid' : 'bad'
                "
      ...
      <div class="bottom-stat">
        <span
          >累计训练
          <strong style="color: #2ecc71">{{ store.records.length }}</strong>
          次</span
        >
      </div>
    </div>
...
.quick-icon-wrap.purple {
  background: rgba(155, 89, 182, 0.12);
}
.quick-icon-wrap.teal {
...
.record-card-icon.good {
  background: rgba(46, 204, 113, 0.12);
}
.record-card-icon.mid {
  background: rgba(255, 107, 53, 0.12);
}
.record-card-icon.bad {
...
.record-card-rate.good {
  color: #2ecc71;
}
.record-card-rate.mid {
  color: #ff6b35;
}
.record-card-rate.bad {
  color: #e74c3c;
}
.record-card-arrow {
  font-size: 18px;
