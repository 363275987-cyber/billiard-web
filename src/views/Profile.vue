...
<template>
  <div class="page">
    ...
    <div v-if="!store.isLoggedIn">
      ...
      <!-- Tab 切换 -->
      <div class="tab-switch" style="margin:-15px 12px 0; position:relative; z-index:1;">
        <button class="tab-btn" :class="{active: authTab === 'login'}" @click="authTab = 'login'">登录</button>
        <button class="tab-btn" :class="{active: authTab === 'register'}" @click="authTab = 'register'">注册</button>
      </div>
      ...
      <div class="card" v-if="authTab === 'login'">
        <div class="input-group">
          ...
          <input class="field" v-model="loginPhone" type="tel" maxlength="11" placeholder="手机号"
      ...
      <div class="card" v-if="authTab === 'register'">
        ...
        <template v-if="regStep === 1">
          <div class="input-group">
            <span class="input-icon">📱</span>
            <input class="field" v-model="regPhone" type="tel" maxlength="11" placeholder="手机号"
                   @input="regPhone = regPhone.replace(/\D/g,'')" />
          </div>
          <div class="input-group">
            <span class="input-icon">🔒</span>
            <input class="field" v-model="regPwd" type="password" placeholder="设置密码（6位以上）" maxlength="32" />
          </div>
          <div class="input-group">
            <span class="input-icon">🔒</span>
        ...
        <template v-if="regStep === 2">
          ...
          <div class="avatar-grid">
            <div class="avatar-option" v-for="(av, i) in systemAvatars" :key="i"
                 :class="{active: regAvatar === av}"
                 @click="regAvatar = av; customAvatarUrl = ''">
              <span class="avatar-emoji">{{av}}</span>
            </div>
            <div class="avatar-option" :class="{active: !!customAvatarUrl}" @click="pickCustomAvatar">
              <span class="avatar-emoji" v-if="!customAvatarUrl">📷</span>
              <img v-else :src="customAvatarUrl" class="avatar-custom-thumb" />
            </div>
            <input type="file" ref="avatarInput" accept="image/*" @change="onAvatarFile" style="display:none" />
          </div>

          <!-- 昵称 -->
          <div class="section-label" style="margin-top:16px;">昵称</div>
          <input class="field nick-field" v-model="regNickName" placeholder="起个名字吧" maxlength="12" />

          <!-- 身份 -->
          <div class="section-label" style="margin-top:16px;">我是</div>
          <div class="role-row">
            <div class="role-btn" :class="{active: regRole === 'student'}" @click="regRole = 'student'">
              <span>👨🎓</span><span>学员</span>
            </div>
            <div class="role-btn" :class="{active: regRole === 'coach'}" @click="regRole = 'coach'">
              <span>🧑🏫</span><span>教练</span>
            </div>
          </div>

          <button class="btn-primary" style="margin-top:20px;" @click="handleCompleteReg">完成注册 🎉</button>
        </template>
      </div>
    </div>
    ...
    <div v-else>
      <div class="profile-hero">
        <div class="avatar-circle" :class="store.isCoach ? 'coach' : 'student'">
          <img v-if="isCustomAvatar" :src="store.userInfo.avatar" class="avatar-img" />
          <span v-else class="avatar-emoji-lg">{{store.userInfo.avatar || '🎱'}}</span>
        </div>
      ...
      <div class="menu-list">
        <div class="menu-item" @click="openEditProfile">
          <span class="menu-icon">✏️</span><span class="menu-text">编辑资料</span><span class="menu-arrow">›</span>
        </div>
      ...
      <div class="modal-sheet">
        ...
        <div class="avatar-grid">
          <div class="avatar-option" v-for="(av, i) in systemAvatars" :key="i"
               ...
               @click="editAvatar = av; editCustomUrl = ''">
            <span class="avatar-emoji">{{av}}</span>
          </div>
          <div class="avatar-option" :class="{active: !!editCustomUrl}" @click="pickEditAvatar">
            <img v-if="editCustomUrl" :src="editCustomUrl" class="avatar-custom-thumb" />
            <span class="avatar-emoji" v-else>📷</span>
          </div>
          <input type="file" ref="editAvatarInput" accept="image/*" @change="onEditAvatarFile" style="display:none" />
        </div>

        <div class="section-label" style="margin-top:16px;">昵称</div>
        <input class="field nick-field" v-model="editNick" placeholder="输入昵称" maxlength="12" />

        <button class="btn-primary" style="margin-top:20px;" @click="saveEdit">保存</button>
      </div>
    </div>
  </div>
</template>
...
function pickCustomAvatar() { avatarInput.value?.click() }
...
async function onAvatarFile(e) {
  ...
  reader.onload = () => { customAvatarUrl.value = reader.result }
  reader.readAsDataURL(compressed)
  e.target.value = ''
}

function cleanupBlobUrl() {
  if (avatarBlobUrl.value) { URL.revokeObjectURL(avatarBlobUrl.value); avatarBlobUrl.value = '' }
}
...
const isCustomAvatar = computed(() => {
  ...
  return av.startsWith('data:') || av.startsWith('blob:') || av.startsWith('http')
})
...
function pickEditAvatar() { editAvatarInput.value?.click() }
...
async function onEditAvatarFile(e) {
  ...
  const reader = new FileReader()
  reader.onload = () => { editCustomUrl.value = reader.result; editAvatar.value = '' }
  reader.readAsDataURL(compressed)
  e.target.value = ''
}
...
.hero {
  ...
  padding: 40px 24px 50px; text-align: center;
}
...
.tab-btn {
  flex: 1; padding: 14px 0; border: none; font-size: 15px; font-weight: 600;
  color: #999; background: transparent; cursor: pointer; transition: all 0.2s;
}
.tab-btn.active { color: #2ecc71; background: rgba(46,204,113,0.06); }

/* 卡片 */
.card { padding: 20px; margin: 12px; background: #fff; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.04); }
...
.field {
  flex: 1; font-size: 15px; padding: 14px 0; background: transparent; border: none; outline: none;
  color: #1a1a2e;
}
.field::placeholder { color: #ccc; }
.nick-field {
  width: 100%; font-size: 15px; padding: 14px; background: #f8f9fa; border-radius: 10px;
  border: 2px solid transparent; outline: none; color: #1a1a2e; transition: border-color 0.2s;
  box-sizing: border-box;
}
.nick-field:focus { border-color: #2ecc71; }
...
.avatar-option {
  ...
  border: 3px solid #f0f0f0; cursor: pointer; transition: all 0.2s; overflow: hidden;
  background: #fff;
}
.avatar-option.active { border-color: #2ecc71; background: rgba(46,204,113,0.06); box-shadow: 0 2px 8px rgba(46,204,113,0.2); }
.avatar-emoji { font-size: 28px; line-height: 1; }
.avatar-custom-thumb { width: 100%; height: 100%; object-fit: cover; }

/* 角色 */
.role-row { display: flex; gap: 12px; }
.role-btn {
  flex: 1; display: flex; align-items: center; justify-content: center; gap: 8px;
  padding: 14px; border-radius: 10px; border: 2px solid #f0f0f0; cursor: pointer;
  font-size: 15px; font-weight: 600; color: #666; transition: all 0.2s;
}
.role-btn.active { border-color: #2ecc71; background: rgba(46,204,113,0.06); color: #2ecc71; }
...
.avatar-circle.coach { background: rgba(52,152,219,0.3); }
.avatar-img { width: 100%; height: 100%; object-fit: cover; }
.avatar-emoji-lg { font-size: 36px; }
.profile-name { display: block; font-size: 20px; font-weight: 700; color: #fff; margin-bottom: 4px; }
.phone-display { display: block; font-size: 14px; color: rgba(255,255,255,0.7); margin-bottom: 8px; }
.role-badge {
...
.modal-sheet {
  ...
  border-radius: 16px 16px 0 0; padding: 20px; max-height: 80vh; overflow-y: auto;
}
.modal-header {
