...
<template>
  <div class="page">
    ...
    <div class="sub-tabs">
      <button :class="{active: tab === 'fav'}" @click="tab = 'fav'">⭐ 收藏的训练 ({{store.myFavList.length}})</button>
      <button :class="{active: tab === 'pub'}" @click="tab = 'pub'">📝 我发布的 ({{store.myPublishedList.length}})</button>
    </div>

    <!-- 收藏的 -->
    <div v-if="tab === 'fav'">
      <div class="project-list" v-if="store.myFavList.length > 0">
        <div class="project-item" v-for="p in store.myFavList" :key="p.id" @click="$router.push('/square-detail/' + p.id)">
          <div class="project-left">
            ...
            <div>
              ...
              <span class="project-meta">by {{p.publisher}} · ⭐ {{p.favs}}</span>
            </div>
          </div>
      ...
      <div class="empty-card" v-else>
        ...
        <button class="btn-primary" style="margin-top:16px;" @click="$router.push('/square')">去广场看看</button>
      </div>
    </div>
    ...
    <div v-if="tab === 'pub'">
      <div class="project-list" v-if="store.myPublishedList.length > 0">
        <div class="project-item" v-for="p in store.myPublishedList" :key="p.id" @click="$router.push('/square-detail/' + p.id)">
          <div class="project-left">
            <span class="category-dot" :class="p.category"></span>
            <div>
              <span class="project-name">{{p.name}}</span>
              <span class="category-tag" :class="p.category">{{categoryLabel(p.category)}}</span>
              <span class="project-meta">👍 {{p.likes}} · ⭐ {{p.favs}} · 👥 {{p.participants}}</span>
            </div>
          </div>
          <span class="item-arrow">›</span>
        </div>
      ...
      <div class="empty-card" v-else>
        ...
        <button class="btn-primary" style="margin-top:16px;" @click="$router.push('/publish')">去发布</button>
      </div>
    </div>
  </div>
</template>
...
.page-top { display: flex; align-items: baseline; padding: 16px 0; gap: 8px; }
...
.page-placeholder { width: 40px; }

.sub-tabs { display: flex; background: #fff; border-radius: 8px; padding: 4px; margin-bottom: 12px; box-shadow: 0 2px 6px rgba(0,0,0,0.04); }
.sub-tabs button {
...
.project-left { display: flex; align-items: center; gap: 10px; flex: 1; }
.category-dot { width: 8px; height: 8px; border-radius: 50%; flex-shrink: 0; }
.category-dot.basic { background: #2ecc71; }
.category-dot.medium { background: #ff6b35; }
.category-dot.advanced { background: #e74c3c; }
.project-name { display: block; font-size: 15px; font-weight: 600; color: #1a1a2e; }
.category-tag { display: inline-block; font-size: 10px; padding: 2px 8px; border-radius: 10px; font-weight: 600; margin-left: 6px; }
...
.project-meta { display: block; font-size: 12px; color: #b0b0b0; margin-top: 3px; }
.item-arrow { font-size: 18px; color: #ddd; }
