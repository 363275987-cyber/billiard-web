  ...
  <div class="page" v-if="project">
    ...
    <div class="action-bar">
      ...
      <button
        ...
        :class="{ faved: store.isProjectFaved(project.id) }"
        @click="handleFav"
      >
        {{ store.isProjectFaved(project.id) ? '⭐ 已收藏' : '☆ 收藏' }}
        {{ project.favs }}
      </button>
    </div>
    ...
    <div class="card">
      <button
        class="btn-add-plan"
        :class="{ added: store.isInCart(project.id) }"
        @click="openParamPanel"
      >
        {{
          store.isInCart(project.id)
            ? '✅ 已加入今日训练计划'
            : '🛒 加入今日训练计划'
        }}
      </button>
...
const liked = ref(
  localStorage.getItem('bt_liked_' + route.params.id) === 'true'
)
...
function categoryLabel(cat) {
  return (
    { basic: '🎯 基础', medium: '💪 进阶', advanced: '🔥 高级' }[cat] || cat
  )
}
