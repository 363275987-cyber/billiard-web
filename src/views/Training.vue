    ...
    <div class="live-overlay" v-if="store.currentTraining && liveMode">
      <div class="live-header">
        ...
        <span class="live-timer-wrap">
          <span
            class="live-timer"
            :class="elapsedSeconds >= 3600 ? 'warn' : ''"
      ...
      <template v-else>
        <div class="live-stats">
          ...
          <div class="live-stat">
            <span
              class="live-stat-num"
              :class="
                liveStats.hitRate >= 70
                  ? 'good'
                  : liveStats.hitRate >= 50
                    ? 'mid'
                    : 'bad'
              "
              >{{ liveStats.hitRate }}%</span
            >
        ...
        <div class="batch-area">
          <div class="batch-row">
            ...
            <div class="batch-stepper">
              ...
              <button
                class="stepper-btn"
                @click="batchCount = Math.min(200, batchCount + 1)"
              >
                +
              </button>
            </div>
        ...
        <div class="live-actions">
          <button
            class="btn-undo"
            @click="store.undoShot()"
            :disabled="liveStats.total === 0"
          >
            ↩️ 撤销
          </button>
          <button class="btn-end-live" @click="handleEndLive">
            🏁 结束训练
          </button>
        </div>
      </template>
    ...
    <template v-if="!liveMode || !store.currentTraining">
      ...
      <div class="section" v-if="studentHomework.length > 0 && !store.isCoach">
        <div class="section-title">
          <div
            class="title-bar"
            style="background: linear-gradient(180deg, #3498db, #2980b9)"
          ></div>
          <span>📝 教练作业</span>
        ...
        <div class="homework-list">
          <div class="homework-card" v-for="hw in studentHomework" :key="hw.id">
            ...
            <button class="btn-load-homework" @click="loadHomeworkToCart(hw)">
              🛒 加入今日计划
            </button>
            <button
              class="btn-done-homework"
              @click="handleCompleteHomework(hw)"
            >
              ✅ 标记完成
            </button>
          </div>
      ...
      <div class="section" style="margin-bottom: 12px">
        <div class="section-title">
          <div
            class="title-bar"
            style="background: linear-gradient(180deg, #9b59b6, #8e44ad)"
          ></div>
          <span>🎯 训练科目</span>
        ...
        <div
          ...
          @click="$router.push('/square-subjects')"
        >
      ...
      <div
        class="section"
        v-if="planItems.length === 0 && studentHomework.length === 0"
      >
        <div class="section-title">
          <div
            class="title-bar"
            style="background: linear-gradient(180deg, #f39c12, #e67e22)"
          ></div>
          <span>推荐训练方案</span>
        </div>
        <div class="rec-grid">
      ...
      <div class="section" v-if="planItems.length > 0">
        <div class="section-title">
          ...
          <span class="section-count"
            >{{ completedCount }}/{{ planItems.length }} 完成</span
          >
        </div>
        <div class="plan-list">
          <div
          ...
          >
            ...
            <div class="plan-right">
              ...
              <template v-if="isProjectDone(item.projectId)">
                ...
                <span
                  class="train-count"
                  v-if="getProjectTrainCount(item.name) > 0"
                >
                  已练{{ getProjectTrainCount(item.name) }}次
                </span>
                <button class="btn-again" @click="handleStart(item)">
                  再练
                </button>
              </template>
              ...
              <template v-else>
                <button
                  ...
                  :disabled="!!store.currentTraining"
                >
                  开始
                </button>
              </template>
      ...
      <div class="save-plan-area" v-if="planItems.length > 0">
        <button class="btn-save-plan" @click="handleSavePlan">
          📌 保存本次计划
        </button>
      </div>
      ...
      <!-- 添加训练按钮 -->
      <div class="add-area">
        <button class="btn-add" @click="showAddPicker = true">
          + 添加训练
        </button>
      </div>
      ...
      <div
        ...
        v-if="showAddPicker"
        @click.self="showAddPicker = false"
      >
        <div class="picker-sheet">
          ...
          <template v-if="store.myProjectList.length > 0">
            <div class="picker-list">
              <div
                class="picker-item"
                v-for="p in store.myProjectList"
                :key="p.id"
                @click="quickTrain(p)"
              >
                <span class="picker-item-name">{{ p.name }}</span>
                <span class="picker-item-cat" :class="p.category">{{
          ...
          >
            <div class="record-left">
              <div
                class="record-icon"
                :class="
                  r.hitRate >= 70 ? 'good' : r.hitRate >= 50 ? 'mid' : 'bad'
                "
              >
              ...
              <div class="record-info">
                ...
                <span class="record-meta"
                  >{{ r.duration }}分钟 · {{ r.totalShots }}杆</span
                >
              </div>
            </div>
          ...
          <div class="homework-card" v-for="hw in coachHomework" :key="hw.id">
            <div class="homework-top">
              ...
              <span
                ...
                :class="hw.completed ? 'done' : 'pending'"
              >
      ...
      <div class="section" v-if="store.planHistory.length > 0">
        ...
        <div class="history-list">
          <div
            class="history-card"
            v-for="plan in store.planHistory"
            :key="plan.id"
          >
            <div class="history-top">
              <span class="history-date">{{ plan.date }}</span>
              <span class="history-meta"
                >{{ plan.items.length }}个项目 · 共{{
                  planTotalDuration(plan)
                }}分钟</span
              >
            </div>
            <div class="history-items">
              <span
                class="history-tag"
                v-for="item in plan.items.slice(0, 5)"
                :key="item.projectId"
              >
                {{ item.name }}
              </span>
              <span class="history-tag more" v-if="plan.items.length > 5"
                >+{{ plan.items.length - 5 }}</span
              >
            </div>
            <div class="history-actions">
              <button class="btn-reorder" @click="handleRestore(plan.id)">
                🔄 再来一次
              </button>
              <button
                class="btn-delete-history"
                @click="handleDeleteHistory(plan.id)"
              >
                删除
              </button>
            </div>
...
const activeProjectName = computed(() => {
  ...
  const item = planItems.value.find(
    (i) => i.projectId === store.currentTraining.projectId
  )
...
function handleEndLive() {
  if (
    !confirm(
      `结束「${activeProjectName.value}」训练？已用时 ${formatTime(elapsedSeconds.value)}`
    )
  )
    return
  stopTimer()
...
.picker-item-cat.basic {
  background: rgba(46, 204, 113, 0.12);
  color: #27ae60;
}
.picker-item-cat.medium {
  background: rgba(255, 107, 53, 0.12);
  color: #e55a2b;
}
.picker-item-cat.advanced {
  background: rgba(231, 76, 60, 0.12);
  color: #e74c3c;
}
.picker-empty {
