...
<template>
  <div class="page">
    ...
    <div class="category-tabs">
      ...
      <button
        :class="{ active: category === 'medium' }"
        @click="category = 'medium'"
      >
        💪 进阶
      </button>
      <button
        :class="{ active: category === 'advanced' }"
    ...
    <div class="sort-bar">
      ...
      <button :class="{ active: sortBy === 'likes' }" @click="sortBy = 'likes'">
        👍 点赞
      </button>
      <button :class="{ active: sortBy === 'favs' }" @click="sortBy = 'favs'">
        ⭐ 收藏
      </button>
      <button :class="{ active: sortBy === 'new' }" @click="sortBy = 'new'">
        🆕 最新
      </button>
    </div>
    ...
    <div class="project-list">
      <div class="project-card" v-for="p in filteredProjects" :key="p.id">
        ...
        <p class="project-desc" @click="$router.push('/square-detail/' + p.id)">
          {{ p.desc }}
        </p>
        <div
          class="project-stats"
        ...
        <div class="card-actions">
          <button
            class="btn-add-plan"
            :class="{ added: store.isInCart(p.id) }"
            @click.stop="openParamPanel(p)"
          >
            {{ store.isInCart(p.id) ? '✅ 已在计划中' : '+ 加入训练计划' }}
          </button>
          <button
            class="btn-fav"
            :class="{ faved: store.isProjectFaved(p.id) }"
            @click.stop="toggleFav(p.id)"
          >
    ...
    <div
      class="param-overlay"
      v-if="showParamPanel"
      @click.self="showParamPanel = false"
    >
      <div class="param-sheet">
        ...
        <div class="param-item">
          ...
          <div class="quick-rate">
            <button
              :class="{ active: paramTarget === 30 }"
              @click="paramTarget = 30"
            >
              30%
            </button>
            <button
              :class="{ active: paramTarget === 50 }"
              @click="paramTarget = 50"
            >
              50%
            </button>
            <button
              :class="{ active: paramTarget === 60 }"
              @click="paramTarget = 60"
            >
              60%
            </button>
            <button
              :class="{ active: paramTarget === 70 }"
              @click="paramTarget = 70"
            >
              70%
            </button>
            <button
              :class="{ active: paramTarget === 90 }"
              @click="paramTarget = 90"
            >
              90%
            </button>
            <button
              :class="{ active: paramTarget === 100 }"
              @click="paramTarget = 100"
            >
              100%
            </button>
          </div>
        ...
        <button class="btn-confirm" @click="confirmAddPlan">
          ✅ 确认加入训练计划
        </button>
      </div>
    </div>
    ...
    case 'hot':
      list.sort(
        (a, b) =>
          ...
          b.favs * 3 +
          b.participants -
          (a.likes * 2 + a.favs * 3 + a.participants)
      )
      break
    case 'likes':
